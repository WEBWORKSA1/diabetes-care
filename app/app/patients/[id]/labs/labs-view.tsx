  ...
  if (labs.length === 0) {
    return (
      <section className="bg-card rounded-2xl border border-border px-6 py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">Add labs from the patient&rsquo;s encounter chart.</p>
      </section>
    );
  }
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4 text-sm flex-wrap">
          <div>
            ...
            <span className="text-muted-foreground ml-1">total results</span>
          </div>
          <div>
          ...
          <div>
            ...
            <span className="text-muted-foreground ml-1">distinct tests</span>
          </div>
        </div>
          ...
          {filtered.map((summary) => {
            ...
            return (
              <section key={summary.test_name} className="bg-card rounded-2xl border border-border overflow-hidden">
                <header
                  className="px-5 py-4 border-b border-border cursor-pointer hover:bg-muted/30 transition-colors"
                        ...
                        {summary.values.map((v) => (
                          <tr key={v.id} className="hover:bg-muted/30 transition-colors">
                            ...
                            <td className="px-5 py-2">
                              {v.is_abnormal === true && (
                                <span className="clinical-badge clinical-badge-high">Abnormal</span>
                              )}
                              {v.is_abnormal === false && (
                                <span className="clinical-badge clinical-badge-good">Normal</span>
                              )}
                              {v.is_abnormal === null && (
